...
#define CACHE_EXCLUSIONS_REGEX @"^(%@(/(HSTS\\.plist|com\\.apple\\.(metal|opengl)(/.*)?|(Databases|Cache)\\.db(-shm|-wal)?|WebKit(/.*)?|fsCachedData))?|Snapshots(/.*)?|tor(/.*)?|com\\.apple\\.WebKit\\.(Networking|WebContent)(/.*)?|start.html)$"
